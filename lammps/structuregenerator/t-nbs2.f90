...
!  structure for 1T-NbS2
!  modified from t-hfs2.f90
   ...
   ! structure parameters, lattice constant and bond length from 
   ! the paper, J. Phys. Chem. C 116, 8983−8999 (2012)
   latconst = 3.30
   bond = 2.45
   ...
   ! the middle layer is purely bent, the inner layer is compressed, the
   ! outer layer is tensiled
   if(ltube)then
      zav = 0.0
      do i = 1, ntot
         zav = zav + zalat(i)
   ...
   write(*,'(''# orientation, ntot = '', a1, i12)')orientation, ntot
   if(ltube)then
   ...
   write(*,'(''# basis vector = '',3f12.4 )')a*0.5*sqrt(3.0),a*0.5,0.0
