...
!  structure for black phosphorus.
...
!   ! exp data from Ref.8 in JAP 107, 093718 (2010); i.e., Physica (Amsterdam)
!   ! 105B, 93 (1981)
!   a1 = 4.376_dp
!   a2 = 3.314_dp
!   a3 = 10.478_dp
...
!   v = 0.1017_dp
   ...
   ! data from first-principles calculations in JAP 107, 093718 (2010)
   a1 = 4.422_dp
   a2 = 3.348_dp
   a3 = 10.587_dp
   u = 0.0821_dp
   ...
   write(*,'(''# ntot = '', i12)')ntot
   if(ltube)then
      write(*,'(''# system = BP tube, length, diameter = '',2f16.4)')lenxyz(1),diameter
   else
      write(*,'(''# system = BP sheet, lenxyz(1:3) = '',3f16.4 )')lenxyz(1:3)
   end if
